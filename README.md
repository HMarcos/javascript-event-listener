...
Um exemplo de como adicionar um `click` pode ser visto logo abaixo:
...
É preciso ressaltar que as funções de *callback* podem ser anônimas ou não. Assim, logo a seguir o exemplo mostra como utilizar uma função de callback com o evento `keydown` do teclado.
...
function callback(event) {
    ...
    alert(`A tecla pressionada foi: ${teclaPressionada}`);
}
```
...
Abaixo pode-se ver um exemplo em que são adicionadas duas funções ao clicar em um botão.

```javascript
myButton = document.querySelector(".myButton");

myButton.addEventListener("click", (event) => {
    alert("Olá! Primeira Função!");
})

myButton.addEventListener("click", (event) => {
    alert("Olá! Segunda Função!");
})
```
...
A seguir tem em que captura-se o elemento HTML em que foi clicado e mostra a posição do mouse em relação a esse elemento tudo isso através do objeto `Event`.

```javascript

myButton = document.querySelector(".myButton");

myButton.addEventListener("click", exibirClickMouse)
...
function exibirClickMouse(event) {
    ...
    alert( ` Click no botao ${botao} na posição: (${x}, ${y})`);
...
Um exemplo de configuração da propagação do tipo capturing pode ser vito abaixo:

```javascript

divExterna.addEventListener("click", alertDivExterna, true);
divInterna.addEventListener("click", alertDivInterna, true);
...
Um exemplo de como utiliza o `stopPropagation` pode ser visto abaixo. No caso, como o tipo de propagação é bubbling, o `alert` da div mais externa não é executado.

```javascript
divExterna.addEventListener("click", alertDivExterna, false);
divInterna.addEventListener("click", alertDivInterna, falsle);
